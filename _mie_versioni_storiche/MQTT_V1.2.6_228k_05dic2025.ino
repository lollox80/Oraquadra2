// OraQuadra V1.2.6  -  By Davide Gatti SURVIVAL HACKING  www.survivalhacking.it
...
//
// V1.2.4 - 01/12/2025 - By Lorenzo Mattioli
...
//
// V1.2.5 - 03/12/2025 - By Lorenzo Mattioli
...
// aggiunta visualizzazione versione firmware in web app e Home Assistant (sensore Firmware Version)
// ottimizzato codice OTA per ridurre dimensione sketch
//
// V1.2.6 - 04/12/2025 - By Lorenzo Mattioli
// aggiunta modalità Natale con albero di Natale verde e palline colorate lampeggianti
// modalità Natale completamente staccata dalla modalità orologio (non mostra l'orario)
// albero di Natale dettagliato con gradazioni di verde per effetto realistico
// 12 palline colorate che lampeggiano come lucine natalizie (intervalli casuali 200-800ms)
// stella in cima all'albero che lampeggia con effetto breathing
// fiocchi di neve animati che cadono dall'alto (6 fiocchi con velocità casuale)
// aggiunto preset 19 per modalità Natale nell'interfaccia web
// aggiornato numero totale preset da 19 a 20 (0-19)
//
//
// Mappatura matrice
...
#define MODE_NATALE 10 // Modalità Natale con albero e palline
#define NUM_MODES   11  
    ...
    <div class="container">
        ...
        <div class="status-card">
            ...
            <div class="status-item">
                ...
                <span class="status-value" id="firmwareVersion" style="color: #4ecdc4;">V1.2.6</span>
            </div>
        </div>
...
void handleStatus() {
    ...
    doc["firmwareVersion"] = "1.2.6";  // Versione firmware
...
void handleSetMode() {
    if (server.hasArg("mode")) {
        ...
        if (newMode < 20) { // 20 preset totali
...
bool currentlySleepTime = isSleepTime();
   ...
   if (testModeEnabled) {
       if (currentMillis - testModeStartTime >= 5000) {
           // Dopo 5 secondi, disattiva modalità test
           testModeEnabled = false;
           EEPROM.write(EEPROM_TEST_MODE_ADDR, 0);
           EEPROM.commit();
           updateDisplay(); // Aggiorna con orario reale
           #if ENABLE_MQTT
           publishMqttState();
           #endif
           Serial.println("Modalità test disattivata, tornato all'orario reale");
       }
   }
   
   // Gestione modalità random (eseguita sempre quando abilitata, indipendentemente dalla modalità)
   if (randomModeEnabled && !displayOff && !testModeEnabled) {
       if (currentMillis - lastRandomChange >= randomChangeInterval) {
           ...
           uint8_t newPreset = random(20);  // 0-19
   ...
   if (!displayOff) {
    if(currentMode == MODE_MATRIX) {
        ...
        updateSlowEffect();
    } else if(currentMode == MODE_NATALE) {
        // ═══ Gestione modalità Natale - V1.2.7 ═══
        updateNataleEffect();
    } else {
        if(currentHour != prevHour || currentMinute != prevMinute) {
            ...
            // ═══ Disattiva lampeggio secondi in modalità Natale - V1.2.6 ═══
            if(currentMinute >= 1 && currentMode != MODE_NATALE) {
                showSeconds(currentSecond, clockColors.getColor());
                FastLED.show();
            }
        }
   ...
   if(isModeButtonPressed && isColorButtonPressed) {
       ...
       if(isModeButtonPressed) {
           if(millis() - lastBothPress > DEBOUNCE_TIME) {
               currentPreset = (currentPreset + 1) % 20;  // Aggiornato a 20 preset (0-19) per includere Digitale e Natale
               applyPreset(currentPreset);
               EEPROM.write(EEPROM_PRESET_ADDR, currentPreset);
               EEPROM.commit();
...
void applyPreset(uint8_t preset) {
    ...
    switch(preset) {
        ...
        case 19:
            currentMode = MODE_NATALE;
            // ═══ Pulisci il buffer LED prima di inizializzare Natale - V1.2.7 ═══
            FastLED.clear();
            FastLED.show();
            nataleInitialized = false;  // ═══ Reset inizializzazione per correggere errore visualizzazione - V1.2.7 ═══
            initNataleEffect();
            break;     
   ...
   // ═══ Non mostrare l'orario per modalità Natale - V1.2.7 ═══
   // La modalità Natale è completamente staccata dalla modalità orologio
   // L'aggiornamento viene gestito nel loop principale
   if (currentMode != MODE_NATALE) {
       updateDisplay();
   }
   // Per MODE_NATALE, l'aggiornamento viene fatto nel loop principale
}
